...
THREE.FloorShape.prototype = {
		...
		for (var i = 0; i < numEdges; i++) { // for each edge
			...
			if (i !== numEdges -1 || drawingPolygon.iteration === 0) {
				...
				if (!drawingPolygon.createChildren) {
					...
					// Simetric
					else if (typeOfCenter <   SHAPE_CHILDREN_SIMETRIC_PROBABILITY
											+ SHAPE_CHILDREN_CENTER_PROBABILITY) {
						prevWeight.push(1);
						prevWeight.push(0);
					}
					// Row
					else if (typeOfCenter <   SHAPE_CHILDREN_ROW_PROBABILITY
											+ SHAPE_CHILDREN_SIMETRIC_PROBABILITY
											+ SHAPE_CHILDREN_CENTER_PROBABILITY) {
						prevWeight.push(.80);
						prevWeight.push(.60);
						prevWeight.push(.40);
					...
					// Right
					else if (typeOfCenter <   SHAPE_CHILDREN_RIGHT_PROBABILITY
											+ SHAPE_CHILDREN_LEFT_PROBABILITY
											+ SHAPE_CHILDREN_ROW_PROBABILITY
											+ SHAPE_CHILDREN_SIMETRIC_PROBABILITY
											+ SHAPE_CHILDREN_CENTER_PROBABILITY) {
						prevWeight.push(1);
	...
	pointInsidePolygon: function (point, polygon) {
		...
		for (var i = 0; i < polygon.length; i++) {
			...
			else*/ if (THREE.BuildingUtils.isBetween(ans[0], point.x, outsidePoint.x) &&
