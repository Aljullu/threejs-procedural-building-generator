...
THREE.FloorGeometry = function ( parameters ) {
	...
	function drawRoof() {
		...
		if (scope.drawRoof) {
				...
				roofFaces = THREE.Shape.Utils.triangulateShape ( scope.shape.points, [] );
...
THREE.FloorGeometry.prototype.move = function ( dir ) {
	this.applyMatrix( new THREE.Matrix4().translate( new THREE.Vector3(-dir.x, -dir.y, 0)));
}
...
THREE.FloorGeometry.prototype.insertWindows = function ( windowsToInsert, windowsSeparation, windowsGroup, windowRepetition ) {
		...
		for (var i = 0, length = this.vertices.length; i < length; i += 2) {
				...
				for (var j = 0; j < numberOfWindows; j++) {
						...
						// Load geometry
						var windowIndex = (windowRepetition === "random") ? Math.floor(THREE.Math.random16() * windowsToInsert.length) : windowPos % windowsToInsert.length;
						...
						// Rotate
						windowGeometry.applyMatrix( new THREE.Matrix4().setRotationFromEuler( new THREE.Vector3(-Math.PI/2, angleHor, 0) ) );
						
						// Move
						windowGeometry.applyMatrix( new THREE.Matrix4().makeTranslation( pos.x, pos.y, pos.z ) );
						
						// Add element
						THREE.GeometryUtils.merge(mergedWindowsGeometries, windowGeometry);
						
						windowPos++;
				}
		}
		
		THREE.GeometryUtils.merge(this, mergedWindowsGeometries);
}
...
THREE.FloorGeometry.prototype.insertRoofElements = function ( elementsToInsert, shape2 ) {
	...
	for (var i = 0; i < elementsToInsert.length; i++) {
		...
		while (!isPositionOk) {
			...
			// Rotate
			geometry.applyMatrix( new THREE.Matrix4().setRotationFromEuler( new THREE.Vector3(-Math.PI/2, THREE.BuildingUtils.randomBetween(-Math.PI, Math.PI), 0) ) );
		...
		THREE.GeometryUtils.merge(this, geometry);
